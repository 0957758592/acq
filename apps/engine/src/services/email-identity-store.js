...
export function createEmailIdentityStore({ model, tenantId = 'default' } = {}) {
  ...
  return {
    async register({ address, provider = 'custom', imapHost = '', imapPort = 993, passwordRef, notes = '' }) {
      ...
      const doc = await model.findOneAndUpdate(
        { tenantId, address },
        { $set: { provider, imapHost, imapPort, notes, secretRefs: { password: passwordRef }, status: 'active' }, $setOnInsert: { tenantId, address } },
    ...
    async list() {
      return (await model.find({ tenantId }).lean()).map(strip);
    },
