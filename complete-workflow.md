...
**Instagram parsing — web (browser) + on-device (T2).** `createInstagramWebSelectors` extracts `profile` / `followers` / `following` / `posts` on the **browser tier** (wired by default in the registry). `createInstagramDeviceSelectors` reads the **app-only follower/liker roster** off the live app's UI dump on the **device tier** — usernames matched by their `com.instagram.android` resource-id (verify-by-fact + overridable; a mismatch yields empty rows), fed through `createDeviceScrapeAdapter` (dump → extract → scroll-until-dry → dedup) and wired automatically when a device provider is supplied to the scrape worker. Both are the same seam pattern as Telegram: real mechanism, selectors are the by-fact input.
...
**AI comments (`content.comment`).** Generates a short, human-sounding comment for a target via the **pluggable LLM** (`openai`/`anthropic`/`gemini`/`openrouter`/custom). Comments deliberately use a **chat model** — default **`gpt-4o-mini`** (overridable per-call via `provider`+`model`, or centrally via `COMMENT_PROVIDER`/`COMMENT_MODEL`) — kept **separate from the system-control/brain LLM** (codex, which drives the platform over the 8 surfaces): codex is for *managing the system*, gpt-4o-mini is for *writing comments*. Resolves the target inline or from the targets DB, builds a context-aware prompt from its handle/bio/recent post, returns the text. Posting is the existing `account.action(comment)`/campaign path (exactly-once). A live call needs an LLM API key.

**The loop:** parser → telemetry (produced) + targets (discovered) → `target.score` → `content.comment` → `account.action(comment)` posts → telemetry measures output → dashboards. **Verified live end-to-end** on real Mongo through the one facade — `scripts/output-loop-live.mjs` (target.import→list→score, telemetry.record→output-max summary, content.comment resolving a real target) — and folded into the full-device run as stage 10 of `scripts/full-workflow.mjs`.
